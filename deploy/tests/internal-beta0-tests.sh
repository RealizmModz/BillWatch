...
sh "$root_dir/deploy/tests/account-deletion-smoke-tests.sh" >/dev/null || fail "account deletion smoke regression suite failed."
...
for script in \
    ...
    smoke-subscription-lifecycle.sh
do
...
account_delete_evidence="$temp_dir/account-delete.state"
write_account_delete_evidence()
{
    evidence_release="${1:-$release}"
    {
        printf 'VERSION=1\n'
        printf 'RESULT=complete\n'
        printf 'RELEASE_SHA=%s\n' "$evidence_release"
        printf 'STARTED_AT_UTC=2026-09-05T00:00:00Z\n'
        printf 'COMPLETED_AT_UTC=2026-09-05T00:01:00Z\n'
        printf 'PASSED_PHASES=account-deletion\n'
    } > "$account_delete_evidence"
    chmod 600 "$account_delete_evidence"
}
write_account_delete_evidence
...
run_beta0()
{
    ...
    BILLWATCH_BETA0_RUN_SUBSCRIPTION="${BILLWATCH_BETA0_RUN_SUBSCRIPTION_VALUE:-true}" \
    BILLWATCH_BETA0_ACCOUNT_DELETE_EVIDENCE_FILE="${BILLWATCH_BETA0_ACCOUNT_DELETE_EVIDENCE_FILE_VALUE-$account_delete_evidence}" \
...
BILLWATCH_BETA0_ACCOUNT_DELETE_EVIDENCE_FILE_VALUE=''
if run_beta0 >/dev/null 2>&1; then fail "runner accepted a complete result without account deletion evidence."; fi
BILLWATCH_BETA0_ACCOUNT_DELETE_EVIDENCE_FILE_VALUE="$account_delete_evidence"

write_account_delete_evidence '2222222222222222222222222222222222222222'
if run_beta0 >/dev/null 2>&1; then fail "runner accepted account deletion evidence from another release."; fi
write_account_delete_evidence

chmod 644 "$account_delete_evidence"
if run_beta0 >/dev/null 2>&1; then fail "runner accepted weakly permissioned account deletion evidence."; fi
chmod 600 "$account_delete_evidence"

ln -s "$account_delete_evidence" "$temp_dir/account-delete-link"
BILLWATCH_BETA0_ACCOUNT_DELETE_EVIDENCE_FILE_VALUE="$temp_dir/account-delete-link"
if run_beta0 >/dev/null 2>&1; then fail "runner accepted symbolic-link account deletion evidence."; fi
BILLWATCH_BETA0_ACCOUNT_DELETE_EVIDENCE_FILE_VALUE="$account_delete_evidence"
...
smoke-subscription-lifecycle.sh|https://api.example.test'
[ "$(cat "$phase_log")" = "$expected_phases" ] || fail "complete run did not execute all live acceptance phases in order."
grep -q 'PASS account-deletion evidence (release matched)' "$temp_dir/full.out" || fail "complete run did not report release-matched deletion evidence."
...
grep -q 'SKIP account-deletion evidence (partial run)' "$temp_dir/partial.out" || fail "partial run did not mark deletion evidence missing."
...
grep -q '^PASSED_PHASES=account-deletion,direct-api,web-bff,admin-authz,access-key,plaid,statement,statement-semantics,subscription$' "$evidence" || fail "evidence omitted a required complete-run phase."
