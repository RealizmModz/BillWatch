...
[ "$(cat "$phase_log")" = "$expected_phases" ] || fail "complete run did not execute all smoke phases in order."
...
if grep -q 'smoke-access-key-lifecycle.sh' "$phase_log" || grep -q 'smoke-statement-lifecycle.sh' "$phase_log"; then
