...
grep -q \
    ...
    "$root_dir/deploy/backup/backup.sh" ||
    fail "backup entrypoint must expose guarded retention application."

grep -q \
    'policy) print_retention_policy ;;' \
    "$root_dir/deploy/backup/backup.sh" ||
    fail "backup entrypoint must expose non-destructive retention policy verification."

grep -q \
    -- '--keep-daily "$retention_keep_daily"' \
    "$root_dir/deploy/backup/backup.sh" ||
    fail "backup retention must preserve an explicit daily window."

grep -q \
    -- '--prune' \
    "$root_dir/deploy/backup/backup.sh" ||
    fail "enabled backup retention must prune unreachable repository data."
...
grep -q \
    '        policy' \
    "$root_dir/deploy/check-backup-policy.sh" ||
    fail "backup policy checker must use the non-destructive policy command."
...
if ! env $retention_env \
    sh "$root_dir/deploy/backup/backup.sh" policy >/dev/null; then
    fail "safe minimum backup retention policy was rejected."
fi

if env $retention_env \
    BILLWATCH_BACKUP_KEEP_DAILY=13 \
    sh "$root_dir/deploy/backup/backup.sh" policy >/dev/null 2>&1; then
    fail "backup retention accepted a daily window below the safety floor."
fi

grep -Fq \
    'OnFailure=billwatch-operations-alert@%n.service' \
    "$root_dir/deploy/systemd/billwatch-backup.service" ||
    fail "backup service must route failures to the operations alert unit."

[ -f "$root_dir/deploy/systemd/billwatch-operations-alert@.service" ] ||
    fail "operations alert systemd template is missing."

grep -Fq \
    'BILLWATCH_OPERATIONS_ALERT_WEBHOOK_URL' \
    "$root_dir/deploy/send-operations-alert.sh" ||
    fail "operations alert sender must read the protected webhook configuration."

grep -Fq \
    'chmod 600 "$curl_config"' \
    "$root_dir/deploy/send-operations-alert.sh" ||
    fail "operations alert sender must protect its temporary curl configuration."

grep -Fq \
    -- '--config "$curl_config"' \
    "$root_dir/deploy/send-operations-alert.sh" ||
    fail "operations alert sender must keep the private webhook URL out of curl argv."

if grep -F \
    -- 'curl "$webhook_url"' \
    "$root_dir/deploy/send-operations-alert.sh" >/dev/null; then
    fail "operations alert sender must not expose the private webhook URL in process arguments."
fi

if grep -Eq \
    'docker compose .*logs|journalctl' \
    "$root_dir/deploy/send-operations-alert.sh"; then
    fail "operations alert sender must not attach service logs to external alerts."
fi
