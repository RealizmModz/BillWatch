...
[ -f "$root_dir/Dockerfile" ] ||
    fail "API Dockerfile is missing."

[ -f "$root_dir/Dockerfile.web" ] ||
    fail "web Dockerfile is missing."

[ -x "$root_dir/deploy/validate-production-env.sh" ] ||
    fail "production preflight is not executable."
...
[ -f "$env_file" ] ||
    fail "the production environment file is missing."
...
api_host=$(awk -F= '$1 == "BILLWATCH_HOST" { print substr($0, length($1) + 2); exit }' "$env_file")
web_host=$(awk -F= '$1 == "BILLWATCH_WEB_HOST" { print substr($0, length($1) + 2); exit }' "$env_file")
...
current_release=$(git -C "$root_dir" rev-parse HEAD 2>/dev/null) ||
    fail "the deployment directory is not a Git checkout."

[ "$current_release" = "$release_id" ] ||
    fail "BILLWATCH_RELEASE_ID does not match the checked-out commit."

[ -z "$(git -C "$root_dir" status --porcelain --untracked-files=normal)" ] ||
    fail "the deployment checkout has uncommitted files."
